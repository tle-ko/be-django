...
class CrewDAO(models.Model):
    ...
    def get_problems(self) -> models.QuerySet[CrewProblemDAO]:
        return CrewProblemDAO.objects.filter(**{CrewProblemDAO.field_name.CREW: self})
...
class CrewProblemDAO(models.Model):
    ...
    def get_submissions(self) -> models.QuerySet[CrewSubmissionDAO]:
        return CrewSubmissionDAO.objects.filter(**{CrewSubmissionDAO.field_name.PROBLEM: self})

    def has_submitted(self, user: User) -> bool:
        return self.get_submissions().filter(**{CrewSubmissionDAO.field_name.USER: user}).exists()


class CrewSubmissionDAO(models.Model):
    ...
    def get_comments(self) -> models.QuerySet[CrewSubmissionCommentDAO]:
        return CrewSubmissionCommentDAO.objects.filter(**{CrewSubmissionCommentDAO.field_name.SUBMISSION: self})

    def get_reviewers(self) -> models.QuerySet[User]:
        reviewer_ids = self.get_comments() \
            .values_list(CrewSubmissionCommentDAO.field_name.CREATED_BY, flat=True) \
            .distinct()
        return User.objects.filter(**{User.field_name.PK+'__in': reviewer_ids})


class CrewSubmissionCommentDAO(models.Model):
